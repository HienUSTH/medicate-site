/***********************
 * TRA CỨU THUỐC – chỉ giữ AI tra cứu thuốc
 * Nguồn: RxNav + openFDA + DailyMed
 * Bỏ phụ thuộc vào tabKho/chatKho
 * Giữ autocomplete + kết quả dạng chat
 ***********************/
...
/* ===== compatibility no-op: web mới không còn tab trợ lý kho ===== */
...
let SUGGEST_POOL = [];
const OFDA_CACHE = new Map();

function buildSuggestPool() {
  const vitamins = [
    'Vitamin A (Retinol)', 'Vitamin B1 (Thiamine)', 'Vitamin B2 (Riboflavine)', 'Vitamin B3 (Niacin)',
  ...
  SUGGEST_POOL = [...new Set(pool)].sort((a, b) => a.localeCompare(b));
}
...
function setupAutocomplete() {
  ...
  input.addEventListener('input', async () => {
    ...
    if (pool.length < 5) {
      ...
      pool = [...new Set([...pool, ...ofda])].sort((a, b) => a.localeCompare(b)).slice(0, 5);
    }
    show(pool);
  });
...
function ensureDrugUI() {
  const host = $('#chatThuoc');
  if (!host) return;

  if (!drugInput()) {
    host.innerHTML = `
      <div class="text-sm text-slate-600 mb-3">
        Nhập tên thuốc hoặc hoạt chất để tra cứu chỉ định, liều dùng, tác dụng phụ, cảnh báo và bảo quản.
      </div>
...
function pushLoadingDrug() {
  ...
  $('#drugResultWrap')?.classList.remove('hidden');
...
async function rxnavApprox(q, max = 8) {
  ...
  const j = await (await fetch(url)).json();
  return j?.approximateGroup?.candidate?.map((c) => c.rxcui).filter(Boolean) || [];
}
...
async function ofdaTry(tokens, rxcui, displayName) {
  ...
  const tryOne = async (query) => {
    ...
    if (rec) return { ...normalizeOpenFDALabel(rec), _source: 'openFDA /drug/label' };
    return null;
  };
...
async function resolveOpenFDAName(raw) {
  ...
  if (!q) return { display_inn: '', rxcui: null, openfda_tokens: [] };

  for (const a of LOCAL_ALIAS) {
    if (a.re.test(q)) {
      return {
        display_inn: a.display,
        rxcui: null,
        openfda_tokens: a.tokens || [a.display.toUpperCase()],
        sources: ['Alias(VN)']
      };
    }
  }

  for (const v of VITAMIN_MAP) {
    if (v.re?.test && v.re.test(q)) {
      const strict = await rxStrict(v.name.replace(/^Vitamin\s+[A-Z0-9]+\s*\(|\)$/gi, '').trim()) || await rxStrict(v.name);
      const rxcui = strict?.rxcui || null;
      return {
        display_inn: (strict?.name || v.name),
        rxcui,
        openfda_tokens: [strict?.name || v.name],
        sources: ['RxNav', 'Vitamin map']
      };
    }
  }
  ...
  if (strict) {
    ...
    return {
      display_inn: strict.name,
      rxcui: strict.rxcui,
      openfda_tokens: Array.from(new Set(tokens)).slice(0, 8),
      sources: ['RxNav']
    };
  }
...
async function drugAnswer(raw) {
  const q = String(raw || '').trim();
  if (!q) throw new Error('Bạn chưa nhập tên thuốc.');
...
async function sendDrugQuery(text) {
  ...
  try {
    ...
    replaceLoadingDrug(id, `<div class="px-3 py-3 rounded-2xl bg-rose-50 border border-rose-200 text-sm text-rose-700">Lỗi: ${esc(String(e.message || e))}</div>`);
  }
}

window.sendDrugQuery = sendDrugQuery;
window.ensureDrugUI = ensureDrugUI;
